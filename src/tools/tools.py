...
BORROWINGS_PATH = os.path.join(BASE_DIR, 'db', 'library_mock_data', 'borrowings.csv')


def get_popular_books(args) -> str:
    """TC1: Lấy danh sách sách được mượn nhiều nhất (Top 10)"""
    try:
        df = pd.read_csv(BOOKS_PATH)
        # Tính số lần mượn = total_copies - available_copies
        df['borrows'] = df['total_copies'] - df['available_copies']
        # Sắp xếp giảm dần và lấy top 10
        top_books = df.nlargest(10, 'borrows')[['book_title', 'borrows']].to_dict('records')
        
        result = [{"title": book['book_title'], "borrows": int(book['borrows'])} for book in top_books]
        return json.dumps(result, ensure_ascii=False)
        
    except Exception as e:
        return f"System Error: Lỗi lấy sách được mượn nhiều - {str(e)}"


def get_user_ledger(args) -> str:
    """TC3: Check sách mượn của một sinh viên"""
    try:
        user_id = args.get('user_id')
        if not user_id:
        ...
        # Lọc các bản ghi mượn của sinh viên này và có status='borrowed'
        user_books = df[(df['student_id'] == user_id) & (df['status'] == 'borrowed')]
        ...
        return f"System Error: Lỗi tra cứu sách mượn của sinh viên - {str(e)}"
